  ...
  for (i = 0; i < height; i++) {
    ...
    for (j = 0; j < width; j++) {
      ...
      if (new_x < 0 || new_x > height) {
        ...
        outimg[index + 3 * j + 0] =
            inimg[(int)(height - new_x - 1) * size + (int)new_y * 3 + 0];
        outimg[index + 3 * j + 1] =
            inimg[(int)(height - new_x - 1) * size + (int)new_y * 3 + 1];
        outimg[index + 3 * j + 2] =
            inimg[(int)(height - new_x - 1) * size + (int)new_y * 3 + 2];
      }
    };
  };
