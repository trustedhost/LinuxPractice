...
#define TOTAL 1 /* 학교종의 전체 계이름의 수 */

int notes[] = {/* 학교종을 연주하기 위한 계이름 */
               391};
//     ,391, 440, 440, 391, 391, 329.63, 329.63, \
...
int musicPlay() {
  ...
  softPwmCreate(SPKR, 0, 2000);
  softPwmWrite(SPKR, 100); /* 톤 출력을 위한 PWM 설정 */

  for (i = 0; i < TOTAL; ++i) {
    softToneWrite(SPKR, notes[i]); /* 톤 출력 : 학교종 연주 */
    delay(280); /* 음의 전체 길이만큼 출력되도록 대기 */
  }
  softPwmWrite(SPKR, 0); /* 톤 출력을 위한 PWM 설정 */
  return 0;
}
